...
def filterTags(attrs):
    ...
    # PLEASE REPLACE "STREETLABE" WITH THE NAME OF THE FIELD CONTAINING THE ROAD NAMES

    name_column = "STREETLABE"  # REPLACE WITH YOUR COLUMN
    ...
    tags.update({'highway': highway_class(attrs['CLASS'])})

    if attrs['CLASS'] == '0':
            tags.update({'access': 'private'})
    elif attrs['CLASS'] == '12':
        tags.update({'access': 'no'})
    elif attrs['CLASS'] == 'S1730':
        tags.update({'service': 'alley'})
    elif attrs['CLASS'] == 'S1740':
        tags.update({'access': 'private'})
    elif attrs['CLASS'] == 'S1750' and attrs['FULLNAME'] == 'Driveway':
        del tags['name']
    elif attrs['CLASS'] == 'S1780':
        tags.update({'service': 'parking_aisle'})

        tags.update({'tiger:linearid': attrs['LINEARID']})
    tags.update({'tiger:source': 'https://www.opendataphilly.org/dataset/street-centerlines'})
